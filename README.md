...
- **動態場地破壞（Arena Mask）**：題目可指定哪幾格地板已碎裂（如 M1S 捕鼠），ArenaMap 視覺化呈現破洞與紅色叉叉
- **多重敵對實體模擬（Multiple Entities）**：除了 Boss 還能呈現分身（如 M1S 模仿貓、P6S 靈魂之影），各自帶獨立的位置與面嚮箭頭
- **視覺化連線編輯（Tethers）**：呈現「M1S 傾盆大貓」等牽引機制，連線採 ID 引用（boss / 分身 / waymark）→ 實體位置變動時自動跟著重繪
- **完整覆蓋 M1S 全機制**：本系統可完美呈現「貓跳平移、模仿貓召喚、捕鼠破地、傾盆大貓牽線」等高難度副本核心元素，schema 與渲染管線經過 Phase 1-3 三輪打磨
- **結算與逐題回顧**：
...
- **三段式 questions 子模式**（Phase 2+）：
  - **安全區**：保留既有 SafeArea 繪圖工具
  - **實體與分身**：拖曳 Boss / 分身位置、新增/刪除分身、編輯面嚮
  - **場地破壞**：點擊網格切換破碎/完好、設定副本 grid（rows × cols）
- **視覺化安全區框選工具**：
  - **圓形**：點擊圓心 → 點擊圓周
  ...
  - Esc / 右鍵取消當前繪製
- **全視覺化連線編輯（Tethers, Phase 3）**：
  - 起終點下拉以 optgroup 分四群（Boss / 分身（動態） / 職能 / 場地標記）
  - 顏色 5 色含色塊預覽（紅 / 藍 / 紫 / 黃 / 綠）
  - 端點為職能時自動標示「（練習時定位玩家）」黃字提示
  - 畫布即時 WYSIWYG 預覽連線；Role 端點以淡虛線示意
...
│   └── grid?            副本層共用的網格切分（rows × cols；row-major index）
│                        用於動態場地破壞機制（如 M1S 捕鼠）
└── (擁有多組)
    ├── Strategy (攻略組)
    ...
    │
    └── Question (題目)
        ├── boss          技能名稱、讀條時間、面嚮、position?（預設 arena.center）
        ├── enemies?      分身陣列（id / name / position / facing）
        │                 - 用於模仿貓、靈魂之影等多實體機制
        ├── arenaMask?    破碎網格 index 陣列（依賴 arena.grid，row-major）
        │                 - 此題哪幾格地板已碎裂
        ├── tethers?      連線陣列（sourceId / targetId / color）
        │                 - sourceId/targetId 為 ID 引用：'boss' | enemy.id | WaymarkId
        │                 - 實體位置變動時連線自動跟著重繪
...
2. **座標系統一** — 場地原點為左上角、y 軸向下（與 DOM/Canvas 一致），邏輯座標 1000×1000；boss / enemies / tethers 端點皆採同一系統
...
7. **動態場地與實體（Phase 1+）** — `boss.position` / `enemies` / `arenaMask` / `tethers` 皆為選填，向下相容舊 dataset（未提供則行為等同於「靜態單王」）
8. **Tether 採 ID 引用而非死座標** — 實體拖曳後連線自動跟著動。Player 端依「`'boss'` → `enemy.id` → WaymarkId」順序解析；Editor 端額外支援 RoleId fallback 至場地中央作示意
...
### 設計亮點：虛擬 Index（Virtual Index）

社群匯入的 JSON 並沒有被列在官方 `assets/data/index.json` 中，但卻能無縫進入「副本選擇 → 攻略 → 職能」整套既有流程。秘訣在於：

```ts
// settings store 的 loadCustomDataset
index.value = {
  ...
  instances: [{
    ...
    dataPath: '<custom-import>',   // 虛擬路徑 - 不會真的去 fetch
    schemaVersion: validated.schemaVersion,
...
**為何這樣設計**：
- `InstanceSelector` 與 `StrategySelector` 等 UI 元件皆消費 `index.instances` 與 `dataset.strategies`，完全不知道資料來源
- 自訂題庫塞一個只含「此單一副本」的虛擬 index，UI 邏輯**零修改**就能渲染
- `selectInstance` action 看到 `isCustomDataset === true` 時跳過 `fetch`（dataPath 是虛擬值），直接用記憶體中的 dataset
- 副本卡片自動顯示「自訂題庫」標示與「清除並回到官方題庫」按鈕

這個模式讓社群題庫與官方題庫共享同一套渲染管線，新增題庫類型（例：未來支援「URL 直接載入」）只需新增一個 action 而不需動 UI。
...
3. 使用三個主模式進行編輯：
   ...
   - **題目編輯**：新增 / 編輯 / 複製 / 刪除題目與解答；進入後可再切三個 questions 子模式：
     - **安全區（預設）**：用圓 / 矩形 / 多邊形工具框選正解區域
     - **實體與分身**：直接在畫布上拖曳 Boss 或分身改變位置；側欄新增 / 刪除分身、編輯面嚮；底部「連線設定」區塊可全視覺化新增 tethers
     - **場地破壞**：先在側欄設定副本 `grid`（如 4×4），畫布即顯示輔助網格；點擊任一格即可切換破碎/完好。**⚠ 注意：grid 設定影響整個副本所有題目；縮小尺寸時會自動清掃所有題目中越界的破碎格 index**（store 端 `updateArenaGrid` 全副本掃描，避免存出 player 拒絕的非法資料）
...
- Pinia store 的 cascading reset 契約（含 questions 子模式自動重置）
