# AGENTS.md - FFXIV 高難度副本機制模擬練習器 專案指南
...
6. **文件一致性與可追溯性 (Documentation Consistency)**：
   * 全面同步：每次執行完畢前，必須完整檢視 `AGENTS.md` 與 `README.md`。
...
8. **語言規範與社群在地化 (Language & Localization Standards)**：
   ...
   * **Git Commit Message 格式必須遵循目前 repo 慣例**：一律採用 `type(scope): 中文描述`，例如 `fix(editor): 修正靜態模式圖示路徑錯誤`、`feat(schema/editor/player): Phase 3.6 Tether 種類（kind）與終點箭頭圖標`。`type` 應依變更性質選用 `feat`、`fix`、`docs`、`refactor`、`test`、`chore` 等；`scope` 必須對應實際影響模組，若跨多個子系統可使用 `editor/player/shared` 這類複合 scope。禁止使用偏離既有歷史風格的自由格式 commit 訊息。
   * **每一筆 Commit 都必須補完整 Description（commit body）**：Description 必須使用繁體中文（台灣用語），並依 repo 既有慣例清楚記錄 Why、主要變更、測試/驗證結果；若該次提交不適合寫測試結果，也必須至少說明影響範圍與相關更新細節。禁止只寫單行標題就提交。
   * **所有執行項目皆必須嚴格使用繁體中文（台灣用語）**：包含規劃、進度回報、Review 摘要、測試說明、提交說明、PR 描述、操作紀錄與交接文件。僅在程式碼、API 名稱、第三方套件名稱、遊戲原文術語或使用者明確指定時，才保留原始語言。
...
12. **準則鎖定 (Principle Locking)**：
    * `AGENTS.md` 的「運作準則」不允許被 AI 自行修改，僅能由使用者發起變更。
